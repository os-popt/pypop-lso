...
# NOT show warning information for gym:
...
class ToeplitzPolicy(object):
    ...
    def __init__(self, n_inputs, n_neurons, n_outputs):
        ...
        self.w1 = np.zeros(self.h_dim + self.ob_dim - 1)
    ...
    def evaluate(self, X):
        ...
        z1 = np.tanh(np.dot(self.W1, X) + self.b1.reshape(self.b1.size, 1))
        z2 = np.tanh(np.dot(self.W2, z1) + self.b2.reshape(self.b2.size, 1))
...
if __name__ == "__main__":
    ...
    if params["env_seed"] >= 0: # for simplicity
        ...
        params["env_seed_train"], params["env_seed_test"], params["optimizer_options"]["seed"] =\
            rng.integers(0, np.iinfo(np.int32).max, (3,))
    
    # conduct experiment for one optimizer on one environment
    experiment = Experiment(params)
    print("******* Environment [{}] + Optimizer [{}]:".format(experiment.env_name, experiment.optimizer.__name__))
