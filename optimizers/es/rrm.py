...
class RestartRm(MuCommaLambda):
    """Restart-based Rank-m Evolution Strategy (R-Rm-ES) for large-scale, black-box optimization.
    ...
    def optimize(self, fitness_function=None):
        ...
        # Here introduce 4 new constant symbols to simplify code
        ...
        while n_evaluations < self.max_evaluations:
            if is_restart:
                is_restart = False
                if n_restart > 0:
                    m = self.rng.uniform(self.lower_boundary, self.upper_boundary, (self.ndim_problem,))
                    start_evaluation = time.time()
                    y = fitness_function(m)
